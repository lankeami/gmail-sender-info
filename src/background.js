...
1. SENDER MISMATCH: Does the display name, subject, OR the body content (including company names, department names, portal names, or "signed by" attributions like "XYZ Corp — Human Resources") claim to be from an organization that the email DOMAIN doesn't belong to? This applies even when the display name itself is generic (e.g., "HR", "IT Support", "Billing") — if the BODY repeatedly asserts a specific company/brand identity (logos, "OLX People Portal", "Microsoft Security Team", etc.) while the sending domain and links are unrelated to that company's real domain, that is a sender mismatch regardless of what the display name says. IMPORTANT: Only the domain matters — the local part before @ (noreply, no-reply, receipts, support, info, hello, team, billing, notifications, alerts, etc.) is irrelevant. Example: "Uber Receipts <noreply@uber.com>" is legitimate because uber.com IS Uber's domain. A personal name like "John" or "Mom" from a consumer email provider is NOT a mismatch — only flag when the domain itself doesn't belong to the brand/organization asserted in the display name OR body.
...
AUTHENTICATION CONTEXT: The email data may include SPF, DKIM, and DMARC results. When all three pass, the sender is cryptographically verified — strongly favor "Ok" unless there are clear phishing indicators. However, passing authentication alone does NOT make a BCC'd empty-body email safe — spammers can have valid SPF/DKIM/DMARC. Weigh authentication together with behavioral signals (BCC status, empty body, unknown sender). If a "gateway.spf" note is present, DKIM/SPF authenticate only that the mail genuinely came from its own sending domain's infrastructure — it does NOT vouch for that domain being a legitimate business or affiliated with any brand the email claims to represent. A fully-authenticated email from an unfamiliar, unrelated domain that impersonates a different company in its content is still highly suspicious.
...
function buildAiUserPrompt(data) {
  ...
  if (data.auth) {
    ...
    if (!data.auth.spf && data.auth.gatewaySpf) {
      lines.push(`Note: no real SPF result for the sender's domain — the message only passed a mail-gateway relay/IP allow-list (gateway.spf=${data.auth.gatewaySpf}), which is NOT proof the sender's domain is legitimate.`);
    }
  }
